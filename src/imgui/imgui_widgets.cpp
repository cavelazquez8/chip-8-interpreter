    ...
    if (held && allow_interaction && grab_h_norm < 1.0f) {
        ...
        if (g.ActiveIdIsJustActivated) {
            ...
            g.ScrollbarClickDeltaToGrabCenter =
                (g.ScrollbarSeekMode == 0.0f) ? clicked_v_norm - grab_v_norm - grab_h_norm * 0.5f
                                              : 0.0f;
        }
...
void ImGui::TextLinkOpenURL(const char* label, const char* url) {
    ...
    if (TextLink(label))
        if (g.IO.PlatformOpenInShellFn != NULL) g.IO.PlatformOpenInShellFn(&g, url);
    SetItemTooltip(
        "%s", url);  // It is more reassuring for user to _always_ display URL when we same as label
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size,
    ...
    if (is_multiline) {
        ...
        bool child_visible = BeginChildEx(label, id, frame_bb.GetSize(), ImGuiChildFlags_Border,
...
bool ImGui::BeginBoxSelect(const ImRect& scope_rect, ImGuiWindow* window, ImGuiID box_select_id,
    ...
    if (ms_flags & ImGuiMultiSelectFlags_BoxSelect2d)
        if (bs->BoxSelectRectPrev.Min.x != bs->BoxSelectRectCurr.Min.x ||
            ...
            bs->UnclipMode = true;
            bs->UnclipRect =
                bs->BoxSelectRectPrev;  // FIXME-OPT: UnclipRect x coordinates could be intersection
                                        // of Prev and Curr rect on X axis.
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size,
    ...
    if (flags & ImGuiMultiSelectFlags_ScopeWindow)  // Mark parent child window as navigable into,
                                                    // with highlight. Assume user will always
