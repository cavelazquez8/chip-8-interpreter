...
#define IMGUI_DEBUG_LOG_DOCKING(...)                                                         \
    do {                                                                                     \
        if (g.DebugLogFlags & ImGuiDebugLogFlags_EventDocking) IMGUI_DEBUG_LOG(__VA_ARGS__); \
    } while (0)
#define IMGUI_DEBUG_LOG_VIEWPORT(...)                                                         \
    do {                                                                                      \
        if (g.DebugLogFlags & ImGuiDebugLogFlags_EventViewport) IMGUI_DEBUG_LOG(__VA_ARGS__); \
    } while (0)
...
enum ImGuiItemFlagsPrivate_ {
    ...
    ImGuiItemFlags_Disabled =
        1 << 10,  // false     // Disable interactions (DOES NOT affect visuals, see
...
enum ImGuiItemStatusFlags_ {
    ...
    ImGuiItemStatusFlags_Visible = 1
                                   << 8,  // [WIP] Set when item is overlapping the current clipping
...
enum ImGuiInputFlagsPrivate_ {
    ...
    ImGuiInputFlags_RepeatUntilOtherKeyPress =
        1 << 7,  // Stop repeating when released OR if any other keyboard key is pressed during the
                 // repeat
...
struct ImGuiNavItemData {
    ...
    ImGuiSelectionUserData
        SelectionUserData;  // I+Mov    // Best candidate SetNextItemSelectionUserData() value.
                            // Valid if (InFlags & ImGuiItemFlags_HasSelectionUserData)
...
struct IMGUI_API ImGuiMultiSelectState {
    ...
    int LastSelectionSize;  // Set by BeginMultiSelect() based on optional info provided by user.
                            // May be -1 if unknown.
...
enum ImGuiDockNodeFlagsPrivate_ {
    ...
    ImGuiDockNodeFlags_NoWindowMenuButton =
        1 << 14,  // Saved // Disable window/docking menu (that one that appears instead of the
                  // collapse button)
    ...
    ImGuiDockNodeFlags_DockedWindowsInFocusRoute =
        1 << 18,  //       // Any docked window will be automatically be focus-route chained
                  ...
                  // because the desirable sharing/inheriting/copy-flag-on-split behaviors are quite
    ...
    ImGuiDockNodeFlags_NoDockingOverOther = 1
                                            << 21,  //       // Disable this node from being docked
...
struct ImGuiViewportP : public ImGuiViewport {
    ...
    float Alpha;  // Window opacity (when dragging dockable windows/viewports we make them
                  // transparent)
    float LastAlpha;
    bool LastFocusedHadNavWindow;  // Instead of maintaining a LastFocusedWindow (which may harder
                                   // to correctly maintain), we merely store weither NavWindow !=
                                   // NULL last time the viewport was focused.
    ...
    ImDrawList* BgFgDrawLists[2];   // Convenience background (0) and foreground (1) draw lists. We
                                    // use them to draw software mouser cursor when
...
struct ImGuiContext {
    ...
    ImGuiID DebugHookIdInfo;  // Will call core hooks: DebugHookIdInfo() from GetID functions, used
    ...
    bool
        HoveredIdIsDisabled;  // At least one widget passed the rect test, but has been discarded by
                              // disabled flag or popup inhibit. May be true even if HoveredId == 0.
    bool ItemUnclipByLog;     // Disable ItemAdd() clipping, essentially a memory-locality friendly
                              // copy of LogEnabled
    ...
    // NewFrame().
    double LastKeyModsChangeTime;  // Record the last time key mods changed (affect repeat delay
                                   // when using shortcut logic)
    double LastKeyModsChangeFromNoneTime;  // Record the last time key mods changed away from being
                                           // 0 (affect repeat delay when using shortcut logic)
    ...
    bool ActiveIdUsingAllKeyboardKeys;  // Active widget will want to read all keyboard keys inputs.
                                        // (this is a shortcut for not taking ownership of 100+
                                        // keys, frequently used by drag operations)
    ImGuiKeyChord
    ...
    ImGuiID
        CurrentFocusScopeId;  // Value for currently appending items == g.FocusScopeStack.back().
    ...
    ImGuiID NavFocusScopeId;  // Focused focus scope (e.g. selection code often wants to "clear
                              // other items" when landing on an item of the same scope)
    ...
    ImGuiPlatformImeData PlatformImeDataPrev;  // Previous frame data. When changed we call the
                                               // io.PlatformSetImeDataFn() handler.
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas) {
        ...
        // Same as DebugBreakClearData(). Those fields are scattered in their respective subsystem
        // to stay in hot-data locations
...
struct IMGUI_API ImGuiWindowTempData {
    ...
    bool NavIsScrollPushableX;  // Set when current work location may be scrolled horizontally when
                                // moving left / right. This is generally always true UNLESS within
                                // a column.
