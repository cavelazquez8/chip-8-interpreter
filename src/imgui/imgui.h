...
// - Gallery .................... https://github.com/ocornut/imgui/issues/7503 (please post your
...
struct ImGuiViewport;     // A Platform Window (always 1 unless multi-viewport are enabled. One per
                          // platform window to output to). In the future may represent Platform
                          // Monitor
...
// About dockspaces:
// - Use DockSpaceOverViewport() to create a window covering the screen or a specific viewport + a
// dockspace inside it.
//   This is often used with ImGuiDockNodeFlags_PassthruCentralNode to make it transparent.
// - Use DockSpace() to create an explicit dock node _within_ an existing window. See Docking demo
// for details.
...
IMGUI_API ImGuiID DockSpace(ImGuiID dockspace_id, const ImVec2& size = ImVec2(0, 0),
...
IMGUI_API void SetNextWindowClass(
    const ImGuiWindowClass*
        window_class);  // set next window class (control docking compatibility + provide hints to
...
IMGUI_API ImGuiViewport* FindViewportByPlatformHandle(
    void* platform_handle);  // this is a helper for backends. the type platform_handle is decided
...
enum ImGuiChildFlags_ {
    ...
    ImGuiChildFlags_NavFlattened =
        1 << 8,  // [BETA] Share focus scope, allow gamepad/keyboard navigation to cross over parent
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_AutoSelectAll = 1
                                        << 12,  // Select entire text when first taking mouse focus
    ...
    ImGuiInputTextFlags_CallbackAlways = 1 << 19,  // Callback on each iteration. User code may
    ...
    ImGuiInputTextFlags_CallbackResize =
        1 << 21,  // Callback on buffer capacity changes request (beyond 'buf_size' parameter
    ...
    ImGuiInputTextFlags_CallbackEdit =
        1 << 22,  // Callback on any edit (note that InputText() already returns true on edit, the
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
        1 << 15,  // (WIP) Nav: left direction may move to this TreeNode() from any of its child
...
enum ImGuiPopupFlags_ {
    ...
    ImGuiPopupFlags_NoOpenOverItems = 1
                                      << 8,  // For BeginPopupContextWindow(): don't return true
...
enum ImGuiHoveredFlags_ {
    ...
    ImGuiHoveredFlags_NoNavOverride =
        1 << 11,  // IsItemHovered() only: Disable using gamepad/keyboard navigation state when
...
enum ImGuiInputFlags_ {
    ...
    ImGuiInputFlags_RouteOverActive =
        1 << 15,  // Option: global route: higher priority than active item. Unlikely you need to
    ...
    ImGuiInputFlags_RouteFromRootWindow = 1
                                          << 17,  // Option: route evaluated from the point of view
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_DpiEnableScaleViewports =
        1
        << 14,  // [BETA: Don't use] FIXME-DPI: Reposition and resize imgui windows when the
                // DpiScale of a viewport changed (mostly useful for the main viewport hosting other
                // window). Note that resizing the main window itself is up to your application.
    ImGuiConfigFlags_DpiEnableScaleFonts =
        1 << 15,  // [BETA: Don't use] FIXME-DPI: Request bitmap-scaled fonts to match DpiScale.
                  // This is a very low-quality workaround. The correct way to handle DPI is
                  // _currently_ to replace the atlas and/or fonts in the Platform_OnChangedViewport
...
enum ImGuiMouseSource : int {
    ...
    ImGuiMouseSource_TouchScreen,  // Input is coming from a touch screen (no hovering prior to
                                   ...
                                   // wheeling possible).
    ImGuiMouseSource_Pen,          // Input is coming from a pressure/magnetic pen (often used in
...
enum ImGuiTableColumnFlags_ {
    ...
    ImGuiTableColumnFlags_NoHeaderLabel =
        1 << 12,  // TableHeadersRow() will not submit horizontal label for this column. Convenient
...
struct ImGuiStyle {
    ...
    float TabBarBorderSize;  // Thickness of tab-bar separator, which takes on the tab active color
                             // to denote focus.
    float TabBarOverlineSize;       // Thickness of tab-bar overline, which highlights the selected
                                    // tab-bar.
    ...
    ImVec2 DisplaySafeAreaPadding;  // Apply to every windows, menus, popups, tooltips: amount where
...
struct ImGuiIO {
    ...
    bool ConfigDockingTransparentPayload;  // = false          // [BETA] Make window or viewport
                                           // transparent when docking and only display docking
                                           // boxes on the target viewport. Useful if rendering of
                                           // multiple viewport cannot be synced. Best used with
                                           // ConfigViewportsNoAutoMerge.

    // Viewport options (when ImGuiConfigFlags_ViewportsEnable is set)
    bool
        ConfigViewportsNoAutoMerge;  // = false;         // Set to make all floating imgui windows
    ...
    bool ConfigViewportsNoDecoration;   // = true           // Disable default OS window decoration
                                       ...
                                       // window decorations, ImGuiViewportFlags_NoDecoration will
                                       // be set on it. Enabling decoration can create subsequent
                                       // issues at OS levels (e.g. minimum window size).
    bool
        ConfigViewportsNoDefaultParent;  // = false          // Disable default OS parenting to main
    ...
    bool ConfigDebugIgnoreFocusLoss;  // = false          // Ignore io.AddFocusEvent(false),
                                      ...
                                      // io.ClearInputKeys()/io.ClearInputMouse() in input
                                      // processing.
    ...
    // on Windows)
    void (*PlatformSetImeDataFn)(ImGuiContext* ctx, ImGuiViewport* viewport,
                                 ImGuiPlatformImeData* data);
    // void      (*SetPlatformImeDataFn)(ImGuiViewport* viewport, ImGuiPlatformImeData* data); //
    // [Renamed to io.PlatformSetImeDataFn in 1.91.0]
    ...
    ImGuiID
        MouseHoveredViewport;  // (Optional) Modify using io.AddMouseViewportEvent(). With
                               ...
                               // _IGNORING_ viewports with the ImGuiViewportFlags_NoInputs flag is
                               // much better (few backends can handle that). Set io.BackendFlags |=
                               // ImGuiBackendFlags_HasMouseHoveredViewport if you can provide this
                               // info. If you don't imgui will infer the value using the rectangles
                               // and last focused time of the viewports it knows about (ignoring
                               // other OS windows).
...
struct ImGuiWindowClass {
    ...
    bool DockingAlwaysTabBar;    // Set to true to enforce single floating windows of this class
                                 // always having their own docking node (equivalent of setting the
                                 // global io.ConfigDockingAlwaysTabBar)
...
struct ImGuiMultiSelectIO {
    ...
    bool RangeSrcReset;  //        app:w     /  ms:r          // (If using deletion) Set before
                         //        EndMultiSelect() to reset ResetSrcItem (e.g. if deleted
                         //        selection).
    int ItemsCount;      //  ms:w, app:r     /        app:r   // 'int items_count' parameter to
                     //  BeginMultiSelect() is copied here for convenience, allowing simpler calls
                     //  to your ApplyRequests handler. Not used internally.
};
...
enum ImGuiSelectionRequestType {
    ...
    ImGuiSelectionRequestType_SetAll,    // Request app to clear selection (if Selected==false) or
                                         ...
                                         // RangeFirstItem/RangeLastItem as its contents is entirely
    ...
    ImGuiSelectionRequestType_SetRange,  // Request app to select/unselect
                                         ...
                                         // on value of Selected. Only EndMultiSelect() request
                                         // this, app code can read after BeginMultiSelect() and it
...
struct ImGuiSelectionRequest {
    ...
    ImGuiSelectionUserData
        RangeFirstItem;  //                  /  ms:w, app:r   // Parameter for SetRange request
                         //                  (this is generally == RangeSrcItem when shift selecting
                         //                  from top to bottom).
    ImGuiSelectionUserData
...
struct ImGuiSelectionBasicStorage {
    ...
    bool PreserveOrder;  // = false  // GetNextSelectedItem() will return ordered selection
                         // (currently implemented by two additional sorts of selection. Could be
                         // improved)
...
struct ImGuiSelectionExternalStorage {
    ...
    void (*AdapterSetItemSelected)(
        ...
        bool selected);  // e.g. AdapterSetItemSelected = [](ImGuiSelectionExternalStorage* self,
                         // int idx, bool selected) { ((MyItems**)self->UserData)[idx]->Selected =
                         // selected; }
...
struct ImDrawList {
    ...
    IMGUI_API void AddEllipse(const ImVec2& center, const ImVec2& radius, ImU32 col,
    ...
    // as a convenience fo user but not used by main library.
...
struct ImFontConfig {
    ...
    float RasterizerDensity;  // 1.0f     // DPI scale for rasterization, not altering other font
                              ...
                              // for a zooming display. IMPORTANT: If you increase this it is
                              // expected that you increase font scale accordingly, otherwise
...
enum ImGuiViewportFlags_ {
    ...
    ImGuiViewportFlags_NoInputs = 1 << 7,      // Platform Window: Make mouse pass through so we can
    ...
    ImGuiViewportFlags_IsMinimized =
        1 << 12,  // Platform Window: Window is minimized, can skip render. When minimized we tend
                  // to avoid using the viewport pos/size for clipping window or testing if they are
...
struct ImGuiViewport {
    ...
    ImDrawData* DrawData;  // The ImDrawData corresponding to this viewport. Valid after Render()
                           // and until the next call to NewFrame().
    ...
    void* PlatformHandle;     // void* to hold higher-level, platform window handle (e.g. HWND,
                              // GLFWWindow*, SDL_Window*), for FindViewportByPlatformHandle().
    ...
    bool PlatformRequestResize;  // Platform window requested resize (e.g. window was resized by the
                                 ...
                                 // size)
    bool PlatformRequestClose;   // Platform window requested closure (e.g. window was moved by the
                                 // OS / host window manager, e.g. pressing ALT-F4)
...
struct ImGuiPlatformMonitor {
    ...
    void* PlatformHandle;      // Backend dependant data (e.g. HMONITOR, GLFWmonitor*, SDL Display
                               // Index, NSScreen*)
