...
//  ImGuiConfigFlags_NoMouseCursorChange'. [X] Platform: Multi-viewport support (multiple windows).
//  Enable with 'io.ConfigFlags |= ImGuiConfigFlags_ViewportsEnable'.
// Issues:
//  [ ] Platform: Multi-viewport: Minimized windows seems to break mouse wheel events (at least
//  under Windows). [ ] Platform: Multi-viewport: ParentViewportID not honored, and so
//  io.ConfigViewportsNoDefaultParent has no effect (minor). [x] Platform: Basic IME support. App
//  needs to call 'SDL_SetHint(SDL_HINT_IME_SHOW_UI, "1");' before SDL_CreateWindow()!.
...
// SDL_CreateWindow().
static void ImGui_ImplSDL2_PlatformSetImeData(ImGuiContext*, ImGuiViewport* viewport,
    ...
    switch (event->type) {
        ...
        case SDL_KEYUP: {
            ...
            io.SetKeyEventNativeData(
                ...
                event->key.keysym
                    .scancode);  // To support legacy indexing (<1.87 user code). Legacy backend
...
static void ImGui_ImplSDL2_UpdateMouseData() {
    ...
    if (is_app_focused) {
        ...
        if (bd->MouseCanUseGlobalState && bd->MouseButtonsDown == 0) {
            ...
            // (0,0) when the mouse is on the upper-left corner of the app window) Multi-viewport
    ...
    // the OS mouse cursor is hovering. If ImGuiBackendFlags_HasMouseHoveredViewport is not set by
    // the backend, Dear imGui will ignore this field and infer the information using its flawed
    // heuristic.
...
static void ImGui_ImplSDL2_UpdateGamepads() {
    ...
    ImGui_ImplSDL2_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickLeft,
                                       SDL_CONTROLLER_AXIS_LEFTX, -thumb_dead_zone, -32768);
    ...
    ImGui_ImplSDL2_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickDown,
                                       SDL_CONTROLLER_AXIS_LEFTY, +thumb_dead_zone, +32767);
...
// FIXME: Note that doesn't update with DPI/Scaling change only as SDL2 doesn't have an event for it
// (SDL3 has).
static void ImGui_ImplSDL2_UpdateMonitors() {
    ImGui_ImplSDL2_Data* bd = ImGui_ImplSDL2_GetBackendData();
...
void ImGui_ImplSDL2_NewFrame() {
    ...
    // testing for payload reduces noise and potential side-effects.
