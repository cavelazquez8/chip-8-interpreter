...
Chip8::Chip8() : lastError_(ErrorCode::None) { 
  init(); 
}
...
void Chip8::emulateCycle() {
  ...
  switch (opcode_ & 0xF000) {
    case 0x0000: handleOpcode0xxx(); break;
    case 0x1000: handleOpcode1xxx(); break;
    case 0x2000: handleOpcode2xxx(); break;
    case 0x3000: handleOpcode3xxx(); break;
    case 0x4000: handleOpcode4xxx(); break;
    case 0x5000: handleOpcode5xxx(); break;
    case 0x6000: handleOpcode6xxx(); break;
    case 0x7000: handleOpcode7xxx(); break;
    case 0x8000: handleOpcode8xxx(); break;
    case 0x9000: handleOpcode9xxx(); break;
    case 0xA000: handleOpcodeAxxx(); break;
    case 0xB000: handleOpcodeBxxx(); break;
    case 0xC000: handleOpcodeCxxx(); break;
    case 0xD000: handleOpcodeDxxx(); break;
    case 0xE000: handleOpcodeExxx(); break;
    case 0xF000: handleOpcodeFxxx(); break;
    default:
      setError(ErrorCode::UnknownOpcode, 
               "Unknown opcode: 0x" + std::to_string(opcode_));
      return;
  }
...
void Chip8::setPixel(std::uint16_t x, std::uint16_t y, std::uint8_t value) {
  if (x >= DISPLAY_WIDTH || y >= DISPLAY_HEIGHT) {
    setError(ErrorCode::InvalidMemoryAccess, 
             "Pixel coordinates out of bounds: (" + std::to_string(x) + ", " + std::to_string(y) + ")");
    return;
  }
...
void Chip8::setKeyState(std::uint8_t key, bool pressed) {
  if (key >= KEYBOARD_SIZE) {
    setError(ErrorCode::InvalidRegisterAccess, 
             "Invalid key index: " + std::to_string(key));
    return;
  }
...
void Chip8::setDelayTimer(std::uint8_t value) { 
  delayTimer_ = value; 
}

void Chip8::setDrawFlag(bool condition) { 
  drawFlag_ = condition; 
}

void Chip8::setIndexRegister(std::uint16_t value) { 
  indexRegister_ = value; 
}
...
std::uint16_t Chip8::getIndexRegister() const { 
  return indexRegister_; 
}

std::uint16_t Chip8::getProgramCounter() const { 
...
std::uint8_t Chip8::getStackPointer() const { 
...
std::uint8_t Chip8::getDelayTimer() const { 
  return delayTimer_; 
}
...
Chip8::ErrorCode Chip8::getLastError() const {
  return lastError_;
}

const std::string& Chip8::getLastErrorMessage() const {
...
void Chip8::handleOpcode3xxx() {
  ...
  if (!isValidRegisterIndex(x)) {
    setError(ErrorCode::InvalidRegisterAccess, 
             "Invalid register index: " + std::to_string(x));
    return;
  }
...
void Chip8::handleOpcodeDxxx() {
  ...
  for (std::uint8_t row = 0; row < height; ++row) {
    if (indexRegister_ + row >= MEMORY_SIZE) {
      setError(ErrorCode::InvalidMemoryAccess, 
               "Sprite data out of memory bounds");
      return;
    }
  ...
  switch (opcode_ & 0x00FF) {
    ...
    case 0x0033: { // 0xFX33 - Store BCD representation of VX
      if (indexRegister_ + 2 >= MEMORY_SIZE) {
        setError(ErrorCode::InvalidMemoryAccess, 
                 "BCD storage out of memory bounds");
        return;
      }
    ...
    case 0x0055: { // 0xFX55 - Store V0 to VX in memory starting at I
      if (indexRegister_ + x >= MEMORY_SIZE) {
        setError(ErrorCode::InvalidMemoryAccess, 
                 "Register dump out of memory bounds");
        return;
      }
    ...
    case 0x0065: { // 0xFX65 - Load V0 to VX from memory starting at I
      if (indexRegister_ + x >= MEMORY_SIZE) {
        setError(ErrorCode::InvalidMemoryAccess, 
                 "Register load out of memory bounds");
        return;
      }
      for (std::uint8_t i = 0; i <= x; ++i) {
...
bool Chip8::isValidMemoryAddress(std::uint16_t address) const {
  return address < MEMORY_SIZE;
}

bool Chip8::isValidRegisterIndex(std::uint8_t index) const {
