...
TEST_F(PerformanceTest, CycleExecutionSpeed) {
    ...
    double cyclesPerSecond = static_cast<double>(numCycles) / 
...
TEST_F(PerformanceTest, MemoryAccessSpeed) {
    ...
    double writesPerSecond = static_cast<double>(numAccesses) / 
                           (static_cast<double>(writeTime.count()) / 1e9);
    double readsPerSecond = static_cast<double>(numAccesses) / 
                          (static_cast<double>(readTime.count()) / 1e9);
...
TEST_F(PerformanceTest, DisplayUpdateSpeed) {
    ...
    std::vector<std::uint8_t> displayRom = {
        0xA2, 0x30,  // I = 0x230
        0x60, 0x00,  // V0 = 0
        0x61, 0x00,  // V1 = 0
        0xD0, 0x15,  // Draw sprite
    ...
    if (drawCount > 0) {
        double drawsPerSecond = static_cast<double>(drawCount) / 
                              (static_cast<double>(duration.count()) / 1e9);
...
TEST_F(PerformanceTest, KeyboardResponseTime) {
    ...
    double testsPerSecond = static_cast<double>(numTests) / 
                          (static_cast<double>(duration.count()) / 1e9);
...
TEST_F(PerformanceTest, ErrorHandlingOverhead) {
    ...
    double validOpsPerSecond = static_cast<double>(numOperations) / 
                             (static_cast<double>(validTime.count()) / 1e9);
    double invalidOpsPerSecond = static_cast<double>(numOperations) / 
                               (static_cast<double>(invalidTime.count()) / 1e9);
...
TEST_F(PerformanceTest, FrameBufferAccess) {
    ...
    double accessesPerSecond = static_cast<double>(numAccesses) / 
                             (static_cast<double>(duration.count()) / 1e9);
...
TEST_F(PerformanceTest, WorstCaseInstructionPerformance) {
    ...
    std::vector<std::uint8_t> drawRom = {
        0xA2, 0x10,  // I = 0x210
    ...
    double drawsPerSecond = static_cast<double>(numDraws) / 
                          (static_cast<double>(duration.count()) / 1e9);
